...
public:
   ...
   bool solve(const SparseMatrix<T>& matrix, const std::vector<T> & rhs, std::vector<T> & result, T& residual_out, int& iterations_out) 
   {
      ...
      if(rho==0 || rho!=rho) {
         printf("*** CRSolver: Crashed early! rho = %f ***\n", rho);
         iterations_out = 0;
         return false;
      }
      ...
      for(iteration=0; iteration<max_iterations; ++iteration){
         ...
         /*if(iteration % 50 == 0) {
            ...
            multiply(fixed_matrix, result, Aresult);
            add_scaled(-1.0, Aresult, r);
         }*/

         residual_out = abs_max(r);

         if(rho==0 || alpha!=alpha) {
            printf("*** CRSolver: Crashed early! rho = %e and alpha = %e ***\n", rho, alpha);
