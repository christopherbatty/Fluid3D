...
public:
   ...
   void set_matrix_factor(const SparseMatrix<T>& J) {
      J_factor = J;
   }
   ...
   bool solve(const SparseMatrix<T>& matrix, const std::vector<T> & rhs, std::vector<T> & result, T& residual_out, int& iterations_out) 
   {
      ...
      form_preconditioner(matrix, J_factor);
      ...
      for(iteration=0; iteration<max_iterations; ++iteration){
         ...
         apply_preconditioner(r, z);
         //copy(r,z); //Switching these two lines disables preconditioning
...
protected:
   ...
   void form_preconditioner(const SparseMatrix<T>& matrix, const SparseMatrix<T>& matrix_J_factor) {
      
#ifdef MODIFIED
      factor_modified_incomplete_cholesky0(matrix, ic_factor);
#else
      if(matrix_J_factor.m ==0)
         factor_incomplete_cholesky0(matrix, ic_factor, min_diagonal_ratio);
      else {
         FixedSparseMatrix<T> fixed_J;
         fixed_J.construct_from_matrix(J_factor);
         factor_incomplete_qr(fixed_J, matrix, ic_factor);
      }
#endif
   }
